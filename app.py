...
app = Flask(__name__)
moment = Moment(app)
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:quelthalas@localhost:5432/fyyurdb'
...
class Venue(db.Model):
    ...
    #add in start times or shows attribute
...
def create_venue_submission():
  ...
  venue_submission = Venue(name=name, city=city, state=state, phone=phone, address=address,
...
def show_artist(artist_id):
  ...
  #try replacing^data 1, 2, 3, etc with variables?/en
...
def create_artist_submission():
  ...
  genres = request.form.get('genres')
  facebook_link = request.form.get('facebook_link')
  artist_submission = Artist(name=name, city=city, state=state, phone=phone,
  genres=genres, facebook_link=facebook_link)
...
def shows():
  ...
  # TODO: (DONE) replace with real venues data.
  #       num_shows should be aggregated based on number of upcoming shows per venue.
  return render_template('pages/shows.html', data = Shows.query.all()
  )
